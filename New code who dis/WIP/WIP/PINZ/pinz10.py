...
class App:
    def __init__(self, root):
        ...
        self.side_panel = tk.Frame(root, bg="#f0f0f0", width=140, height=400)
    ...
    def update_side_panel(self, event):
        ...
        customFont = tkFont.Font(size=10)  # Change the font size here
        ...
        if selected_item:
            ...
            if record:
                ...
                for i, (label, value) in enumerate(zip(labels, (record[1], record[2], record[3], record[5], record[6], record[7], record[8], record[9], record[12], record[14],  record[16]))):
                    display_text = tk.Text(self.side_panel, wrap=tk.WORD, width=30, height=2, font=customFont)
                    display_text.insert(tk.END, f"{label}: {value if value else 'N/A'}")
                    display_text.config(state=tk.DISABLED, bg=self.side_panel['bg'], bd=0, padx=0, pady=0)  # Make it look like a label
                    display_text.pack(anchor="w")
