...
class App:
    ...
    def update_side_panel(self, event):
        ...
        if selected_item:
            ...
            if record:
                labels = ["Serial Number", "Status", "Date Received", "Received From", "Condition Received", "Processor Loaded", "Date Left", "Sent To","RMA Number", "Date Left", "Reason for RMA"]
                for i, (label, value) in enumerate(zip(labels, (record[1], record[2], record[3], record[5], record[6], record[7], record[8], record[9], record[14], record[15], record[16]))):
    ...
    def edit_entry_form(self):
        ...
        for i, field in enumerate(editable_fields, start=1):  # Starting index from 1 because 0 is for the Serial Number
            ...
            if field == "Status":
                ...
                processor_options = ["First Data/Concord", "RBS/World Pay/5th3rd"]  # Replace with your actual processors
                processor_combobox = ttk.Combobox(top, values=processor_options, textvariable=entry_var)
                processor_combobox.grid(row=i, column=1)
            else:
                tk.Entry(top, textvariable=entry_var).grid(row=i, column=1)
