...
class App:
    ...
    def edit_entry_form(self):
        ...
        conn = sqlite3.connect("pinpads.db")
        cursor = conn.cursor()
        cursor.execute("SELECT status, sent_to, processor_loaded_when_left FROM pinpads WHERE serial_number = ?", (selected_serial_number,))
        ...
        for i, (field, existing_value) in enumerate(zip(editable_fields, existing_data), start=1):
            tk.Label(top, text=field).grid(row=i, column=0)

            entry_var = tk.StringVar()
            entry_var.set(existing_value)  # Auto-fill with existing data
            entry_vars.append(entry_var)
