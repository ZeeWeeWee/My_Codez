...
def add_entry():
    ...
    c.execute("INSERT INTO stores (store_name, main_contact, cell_phone, credit_processor, address, store_phone, contract_type, notes) VALUES (:store_name, :main_contact, :cell_phone, :credit_processor, :address, :store_phone, :contract_type, :notes)",
              {'store_name': entries['Store Name'].get(),
               ...
               'credit_processor': entries['Credit Processor'].get(),
...
# Remove 'Contract Type' from fields list
fields = ['Store Name', 'Main Contact', 'Cell Phone', 'Credit Processor', 'Address', 'Store Phone', 'Notes']
entries = {}
row_num = 0
col_num = 0
...
# Manually add ComboBox for 'Contract Type'
contract_type_var = tk.StringVar()
tk.Label(entry_frame, text="Contract Type", width=20, anchor='e').grid(row=row_num, column=col_num)
contract_type_combobox = ttk.Combobox(entry_frame, textvariable=contract_type_var, values=["Full Contract", "Software Only", "Fee Only", "DNS"])
contract_type_combobox.grid(row=row_num, column=col_num + 1)

# Increment row_num for the next widget
row_num += 1
