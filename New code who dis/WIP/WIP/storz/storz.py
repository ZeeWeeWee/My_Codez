...
login_lock = threading.Lock()  # Lock to synchronize login

def threaded_download_db():
    threading.Thread(target=download_db).start()

def threaded_upload_db():
    threading.Thread(target=upload_db).start()

# Global variables
ftp_username = None
ftp_password = None
ftp_attempt_failed = False  # New variable to indicate failed FTP login


login_attempts = 0

def show_login_dialog():
    global ftp_username, ftp_password, ftp_attempt_failed, login_lock

    login_lock.acquire()  # Acquire the lock
    ...
    def on_ok():
        global ftp_username, ftp_password, login_attempts, login_lock
        ftp_username = username_entry.get()
        ftp_password = password_entry.get()
        login_attempts = 0  # Reset login attempts
        login_lock.release()  # Release the lock
        login_dialog.destroy()

    def cancel_login():
        global ftp_attempt_failed, login_lock
        ftp_attempt_failed = True  # Indicate that the FTP attempt failed
        login_lock.release()  # Release the lock
        login_dialog.destroy()
...
def connect_to_ftp():
    global ftp_username, ftp_password, ftp_attempt_failed, login_attempts, login_lock

    while ftp_username is None or ftp_password is None:
        if ftp_attempt_failed or login_attempts >= MAX_LOGIN_ATTEMPTS:
            return None

        login_attempts += 1
        show_login_dialog()

        login_lock.acquire()  # Wait for the lock to be released
        login_lock.release()  # Immediately release it for next iteration

    try:
        ftp = FTP('councilofheresy.com')  # Replace with your FTP server address
        ftp.login(ftp_username, ftp_password)
        return ftp
    except Exception as e:
        messagebox.showerror("FTP Error", f"Could not connect to FTP: {e}")
        return None
...
def download_db():
    ...
    if ftp is None:
        ...
        ftp = connect_to_ftp()
        if ftp is None:
            messagebox.showerror("FTP Error", "Could not connect to FTP.")
            return
...
def upload_db():
    ...
    try:
        ...
        messagebox.showerror("FTP Error", "Could not upload database.")
...
def setup_database():
    ...
    try:
        c.execute("ALTER TABLE stores ADD COLUMN dns_status BOOLEAN")
...
def add_entry():
    conn = sqlite3.connect("store_info.db")
    c = conn.cursor()
    c.execute("INSERT INTO stores (store_name, main_contact, cell_phone, credit_processor, address, store_phone, contract_type, notes, dns_status) VALUES (:store_name, :main_contact, :cell_phone, :credit_processor, :address, :store_phone, :contract_type, :notes, :dns_status)",
              {'store_name': entries['Store Name'].get(),
               ...
               'dns_status': dns_var.get()})  # New dns_status field
    conn.commit()
    conn.close()
    clear_fields()
    display_records()
...
def display_records(query=None):
    ...
    for record in records:
        item = tree.insert("", tk.END, values=record)
        if record[-1] == 1:  # dns_status is the last column, checking for 1 as True
            tree.tag_configure('dns', background='red')  # Configure the tag
            tree.item(item, tags='dns')  # Apply the tag to the item
...
def populate_fields(event):
    ...
    # Debug print line
    #print(f"Debug DNS: {selected_values[-1]} - type: {type(selected_values[-1])}")
    ...
    # Update DNS checkbox based on the debug output
    if selected_values[-1] == 'None' or selected_values[-1] == '0':
        dns_var.set(False)
...
def edit_entry():
    ...
    c.execute("""UPDATE stores SET store_name=?, main_contact=?, cell_phone=?, credit_processor=?, address=?, store_phone=?, contract_type=?, notes=?, dns_status=? WHERE id=?""",
          (entries['Store Name'].get(), entries['Main Contact'].get(), entries['Cell Phone'].get(),
           credit_processor_var.get(), entries['Address'].get(), entries['Store Phone'].get(),
           contract_type_var.get(), entries['Notes'].get("1.0", tk.END).strip(), dns_var.get(), selected_id))
...
for field in fields[:-1]:  # Exclude the last field 'Notes'
    ...
    if field == 'Credit Processor':
        ...
        entries[field] = ttk.Combobox(entry_frame, textvariable=contract_type_var, values=["Full Contract", "Full Contract Depot", "Software Only", "Fee Only", "Billable", "Maintenance", "COD", "DNS"])
    else:
...
# New Buttons for FTP Operations
ttk.Button(button_frame, text="Download DB", command=threaded_download_db).grid(row=0, column=5, padx=10)
ttk.Button(button_frame, text="Upload DB", command=threaded_upload_db).grid(row=0, column=6, padx=10)
...
# Add DNS checkbox
dns_var = tk.BooleanVar()  # New Variable for DNS
dns_checkbutton = tk.Checkbutton(entry_frame, text="DNS", variable=dns_var)  # New Checkbox for DNS
dns_checkbutton.grid(row=row_num + 1, column=0, columnspan=2)  # Positioning the new DNS Checkbox
