...
def show_context_menu(event):
    if event.state & 0x4 and event.state & 0x1:  # Ctrl and Shift keys are held
...
# Function to handle Ctrl key release
def on_key_release(key):
    global ctrl_pressed
    if key == keyboard.Key.ctrl:
        ctrl_pressed = False

# Function for Special Feature 1
def show_special_feature1():
    try:
        log_file_path = os.path.join("C:\\Buttonz", "program_log.txt")
        log_directory = os.path.dirname(log_file_path)
        ...
        with open(log_file_path, "a") as log_file:
            log_file.write("Special Feature 1 was executed.\n")
            # Capture the standard output and redirect it to the log file
            original_stdout = sys.stdout
            sys.stdout = log_file
            
            # Implement the actual logic of Special Feature 1 here
            print("Special Feature 1 executed")
            
            # Restore the standard output
            sys.stdout = original_stdout
    except Exception as e:
        print(f"Error in Special Feature 1: {e}")

# Function for Special Feature 2
def show_special_feature2():
    try:
        if ctrl_pressed:
            with open(log_file_path, "a") as log_file:
                log_file.write("Special Feature 2 was executed.\n")
                # Capture the standard output and redirect it to the log file
                original_stdout = sys.stdout
                sys.stdout = log_file
                
                # Implement the actual logic of Special Feature 2 here
                print("Special Feature 2 executed")
                
                # Restore the standard output
                sys.stdout = original_stdout
    except Exception as e:
        print(f"Error in Special Feature 2: {e}")
...
def set_pos1_ip_and_dns():
    ...
    secondary_ip_address = "192.168.9.111"  # Replace with the desired secondary IP address
    ...
    if messagebox.askyesno("Confirm Changes", confirm_message):
        set_network_ip(primary_ip_address, subnet_mask, interface_name)

        # Ask user if they want to set the default gateway
        if messagebox.askyesno("Set Default Gateway", f"Do you want to set the Default Gateway to {default_gateway}?"):
            set_default_gateway(default_gateway, interface_name)

        add_secondary_ip(secondary_ip_address, subnet_mask, interface_name)  # New function to add secondary IP
...
def set_default_gateway(gateway_address, interface_name):
    try:
        args = ['netsh', 'interface', 'ipv4', 'set', 'address', f'name="{interface_name}"', 'gateway=', gateway_address, 'gwmetric=0']
        ctypes.windll.shell32.ShellExecuteW(None, "runas", " ".join(args), None, None, 1)
        messagebox.showinfo("Set Default Gateway", "Default Gateway set successfully.")
    except subprocess.CalledProcessError:
        messagebox.showerror("Error", "Error setting Default Gateway.")
...
#button_pos1_ip_and_dns = tk.Button(root, text="Set POS1 IP and DNS", command=set_pos1_ip_and_dns)
#button_pos1_ip_and_dns.grid(row=4, column=0, padx=10, pady=10)

#button12 = tk.Button(root, text="Danger Noodle", command=run_custom_exe_2)
