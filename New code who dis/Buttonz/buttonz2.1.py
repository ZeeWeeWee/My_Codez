...
Epson = tk.Button(root, text="EpsonOPOS", command=run_custom_exe5)
Epson.grid(row=2, column=4, padx=10, pady=10)

clear_read_only = tk.Button(root, text="Clear Read Only(OpenEPS)", command=run_custom_exe4)
clear_read_only.grid(row=3, column=3, padx=10, pady=10)

mute_button = tk.Button(root, text="Volume", command=toggle_mute)
mute_button.grid(row=3, column=0, padx=10, pady=5)

dual_test = tk.Button(root, text="Dual test", command=run_dualtest)
dual_test.grid(row=3, column=1, padx=10, pady=5)
...
all_buttons.append(dual_test)
all_buttons.append(clear_read_only)
all_buttons.append(Epson)
...
def show_context_menu(event):
    if keyboard.is_pressed('ctrl+shift'):  # Check if Ctrl is held down
        context_menu.post(event.x_root, event.y_root)
